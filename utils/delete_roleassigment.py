...
def delete_roleassigment_command(config, pids_file, role_assignee, role_alias):
    ...
    batch_process(pids, lambda pid: delete_roleassigment_action(config['dataverse']['server_url'], config.DATAVERSE_API_TOKEN, pid, role_assignee, role_alias), config['files']['output_dir'], delay=1.5)
...
if __name__ == '__main__':
    ...
    delete_roleassigment_command(config, args.pids_file, role_assignee, role_alias)
